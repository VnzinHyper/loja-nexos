...
function fazerLogin(){const e=$('loginEmail').value.trim().toLowerCase(),p=$('loginPass').value;if(!e.includes('@')||!p){toast('⚠️ E-mail e senha');return}
 if(Store.online){
  Store.pub({action:'upsertUser',email:e,pass:p,refBy:localStorage.getItem('nexos_ref')||''}).then(r=>{
...
// busca o status real de cada pedido no banco e atualiza a tela
async function syncMeusPedidos(){
 if(!Store.online) return;
 const em=session(); if(!em.includes('@')) return;
 try{
  const ids=[];
  for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith('nexos_tok_')&&!k.endsWith('_local'))ids.push({id:k.slice(11),token:localStorage.getItem(k)});}
...
function confirmarPagamento(){
 ...
 fecharModais();
 if(Store.online){
  Store.pub({action:'createOrder',id:orderId,email,items:itens,total,pay:payMethod,pix,ref}).then(r=>{
   currentOrder=orderId;localStorage.setItem('nexos_tok_'+orderId,r.token);
   cart=[];cupomDesc=0;saveCart();renderAll();
   mostrarSucesso(orderId,total,email,pix,r.token);
  }).catch(e=>{
   toast('⚠️ '+(e.message||'Falha ao registrar pedido'));
   salvarLocal(orderId,email,itens,total,payMethod,pix,ref);
  });
 }else{
  salvarLocal(orderId,email,itens,total,payMethod,pix,ref);
 }
}
function salvarLocal(orderId,email,itens,total,payMethod,pix,ref){
 DB=Store.load();
 const order={id:orderId,email,items:itens,total,pay:payMethod,date:new Date().toLocaleString('pt-BR'),status:'aguardando',pix,ref,keys:[]};
 ...
 currentOrder=orderId;localStorage.setItem('nexos_tok_'+orderId,'local');
 cart=[];cupomDesc=0;saveCart();renderAll();
 mostrarSucesso(orderId,total,email,pix,'local');
}
function mostrarSucesso(orderId,total,email,pix,token){
 $('sucessoMsg').innerHTML=`Pedido <b>${orderId}</b> de <b>${BRL(total)}</b> criado via <b>${payMethod==='pix'?'Pix':'Cartão'}</b>.<br>${payMethod==='pix'?`Pague o QR e clique em “Já paguei”.<br>📝 Na <b>descrição do Pix</b> escreva a referência <b>${orderId}</b>.`:'Pagamento do cartão em análise.'}<br><small>${Store.online?'Pedido registrado para a loja.':'⚠️ Modo local: seu staff ainda não conectou o banco.'}</small>`;
...
// observa o pedido: quando o staff entregar, a tela atualiza sozinha
let watchInt=null;
...
renderAll();
try{simular()}catch(e){}
// tenta conectar ao banco compartilhado e, se conseguir, atualiza a loja
Store.hydrate().then(async ok=>{
 if(ok){DB=Store.load();PRODUCTS=DB.products;CATS=DB.categories;applySettings();renderAll();}
 // clique de afiliado tambem vai para o banco
 const ref=localStorage.getItem('nexos_ref');
 if(ref&&ok) Store.pub({action:'click',code:ref}).catch(()=>{});
});
// mantem o catalogo atualizado sem recarregar a pagina
